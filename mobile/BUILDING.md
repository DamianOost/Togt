...
The vc4 candidate profile additionally requires the independently inspected
runtime asset and build manifest to report both `groundedMomentumShell: true`
and `customerFlagship: true`. Device/deep-link smoke must prove the Grounded
Address route is mounted, legacy `RequestMatch` and `BookingForm` are not
registered, and the valid-form legacy link
`togt://customer/workers/00000000-0000-4000-8000-000000000001/book` is
rejected. A recovery build retains these safe routing flags by default. Any
exception requires separate approval and evidence that legacy booking/match
submission is unreachable on both client and server.
...
1. independently verify APK/build-manifest identity, manifest delta, signature,
   signer, permissions, provider metadata, required candidate-profile flags
...
upgrade over the exact grandfathered vc3 baseline, paired synthetic-role smoke,
sanitized ADB evidence, immutable artifact publication, and rollback
verification.
