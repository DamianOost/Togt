...
const { applyGroundedOpenApi } = require('./openapiGrounded');
...
const spec = {
  ...
  info: {
    ...
    description: [
      ...
      '- Canonical Grounded Momentum errors are RFC 9457 problem+json with stable `type` URIs at `' + TYPE_BASE + '/errors/<slug>`. Legacy/auth validation may retain its documented JSON `{ error }` envelope.',
      '- Canonical consequential commands require an `Idempotency-Key` header (UUID v4 recommended). Retries with the same key + input return the stored response with `Idempotent-Replay: true`; changed input is rejected.',
      '- Revisioned Project, fulfilment, trust and Worker commands also require `If-Match` from the latest response `ETag`.',
      ...
      '- Customer data safety: public labourer search returns approximate coordinates only. Legacy-only bookings reveal participant contact and Worker job detail only while `accepted` or `in_progress`. Canonical fulfilment additionally requires non-revoked route access. Customers can always see the address they entered.',
    ].join('\n'),
  ...
  components: {
    ...
    schemas: {
      ...
      LabourerCandidate: {
        type: 'object',
        description: 'Public labourer search result. Exact live coordinates (`current_lat`, `current_lng`) are intentionally not exposed; use approximate coordinates only when present.',
        properties: {
          user_id: { type: 'string', format: 'uuid' },
          name: { type: 'string' },
          hourly_rate: { type: 'number' },
          rating_avg: { type: 'number' },
          approx_lat: { type: ['number', 'null'], description: 'Rounded approximate current latitude, not enough for doorstep-level tracking.' },
          approx_lng: { type: ['number', 'null'], description: 'Rounded approximate current longitude, not enough for doorstep-level tracking.' },
          location_precision: { type: ['string', 'null'], enum: ['approximate', null] },
          distance_km: { type: 'number' },
        },
      },
      MatchRequest: {
        type: 'object',
        description: 'Customers see their own entered address/location. Pinged labourers see approximate coordinates only and should use the resulting Booking or Project resource; canonical fulfilment never reveals exact job data until route access is granted and remains active.',
        properties: {
          id: { type: 'string', format: 'uuid' },
          ...
          hours_est: { type: ['number', 'null'] },
          notes: { type: ['string', 'null'] },
          status: { type: 'string', enum: ['pending', 'matched', 'expired', 'cancelled'] },
          expire_reason: { type: ['string', 'null'], description: 'no_candidates | all_declined | all_timeout | server_restart | error' },
          matched_booking_id: { type: ['string', 'null'], format: 'uuid' },
          matched_labourer_id: { type: ['string', 'null'], format: 'uuid' },
      ...
      Booking: {
        type: 'object',
        description: 'Viewer-dependent booking shape. Customers keep their entered address/location. For legacy-only bookings, counterpart contact and Worker exact job fields are available while status is `accepted` or `in_progress`. If canonical fulfilment policy or scope exists, those fields require canonical route access to have been granted and not revoked; closed or revoked bookings hide them immediately. Labourer views may otherwise include approximate coordinates.',
        properties: {
          id: { type: 'string', format: 'uuid' },
          customer_id: { type: 'string', format: 'uuid' },
          ...
          location_lat: { type: 'number' },
          location_lng: { type: 'number' },
          approx_lat: { type: ['number', 'null'] },
          approx_lng: { type: ['number', 'null'] },
          location_precision: { type: ['string', 'null'], enum: ['approximate', null] },
          scheduled_at: { type: 'string', format: 'date-time' },
          hours_est: { type: ['number', 'null'] },
          total_amount: { type: ['string', 'null'], description: 'Decimal as string (NUMERIC(10,2)).' },
          notes: { type: ['string', 'null'] },
          customer_phone: { type: ['string', 'null'], description: 'Only present to labourers while `accepted` or `in_progress`; canonical fulfilment additionally requires non-revoked route access.' },
          labourer_phone: { type: ['string', 'null'], description: 'Only present to customers while `accepted` or `in_progress`; canonical fulfilment additionally requires non-revoked route access.' },
          labourer_current_lat: { type: ['number', 'null'], description: 'Fresh exact labourer live latitude, only present to the customer after contact reveal.' },
          labourer_current_lng: { type: ['number', 'null'], description: 'Fresh exact labourer live longitude, only present to the customer after contact reveal.' },
          start_pin_required: { type: 'boolean', description: 'Legacy compatibility only. Always false when canonical fulfilment applies.' },
          ready_to_start: { type: 'boolean', description: 'Legacy compatibility only. Always false when canonical fulfilment applies.' },
          canonical_start_required: { type: ['boolean', 'null'], description: 'True when clients must use the canonical Project start command.' },
          start_path: { type: ['string', 'null'], description: 'Canonical Project start command path when canonical fulfilment applies.' },
          created_at: { type: 'string', format: 'date-time' },
        },
      },
      WebhookSubscription: {
        ...
        properties: {
          ...
          secret_previous_expires_at: { type: ['string', 'null'], format: 'date-time', description: 'Non-null while a previous secret is still valid (24h grace post-rotation).' },
        },
      },
      ...
      WebhookDelivery: {
        ...
        properties: {
          ...
          dead_at: { type: ['string', 'null'], format: 'date-time' },
        },
      },
    },
    examples: {
      BookingPendingForLabourer: {
        summary: 'Pending labourer view hides exact customer data',
        value: {
          id: '00000000-0000-0000-0000-000000000010',
          customer_id: '00000000-0000-0000-0000-000000000001',
          labourer_id: '00000000-0000-0000-0000-000000000002',
          status: 'pending',
      ...
      BookingAcceptedForLabourer: {
        summary: 'Accepted labourer view reveals exact customer job details',
        value: {
          id: '00000000-0000-0000-0000-000000000010',
          status: 'accepted',
          skill_needed: 'Plumbing',
          address: '12 Example Street, Cape Town',
      ...
      MatchCandidateForLabourer: {
        summary: 'Match candidate view uses approximate location',
        value: {
  ...
  paths: {
    ...
    '/auth/refresh': {
      post: {
        ...
        description: 'A duplicate request observed after another request completed the same rotation returns 409 for the five-second rotation grace window and does not revoke the winning session. Reuse after that window returns 401 and revokes the active token family.',
        security: [],
        requestBody: {
          required: true,
        ...
        responses: {
          ...
          '401': { $ref: '#/components/responses/Problem401' },
          '409': {
            description: 'This token was already rotated by a concurrent request during the five-second grace window. The client must use its newer saved session or sign in again; retrying this token cannot succeed.',
            content: {
              'application/json': {
                schema: {
                  type: 'object',
                  ...
                  properties: {
                    error: { type: 'string', const: 'refresh_rotation_already_completed' },
                    detail: { type: 'string' },
              ...
              'application/json': {
                schema: {
                  ...
                  properties: {
                    ...
                    attempts: { type: 'array', items: { type: 'object', properties: { id: { type: 'string', format: 'uuid' }, labourer_id: { type: 'string', format: 'uuid' }, status: { type: 'string', enum: ['pinged', 'accepted', 'declined', 'timeout', 'cancelled'] }, pinged_at: { type: 'string', format: 'date-time' }, responded_at: { type: ['string', 'null'], format: 'date-time' } } } },
                  },
                },
              },
            },
              ...
              'application/json': {
                schema: {
                  ...
                  properties: {
                    ...
                    name: { type: 'string' },
                    id_last4: { type: ['string', 'null'], description: 'Last four digits of the SA ID. Raw ID and derived DOB/sex/citizenship fields are not returned.' },
                    vendor: { type: ['object', 'null'], properties: { request_id: { type: 'string' } } },
                  },
                },
              },
            },
  ...
  'x-error-types': [
    ...
    { type: 'refresh_rotation_already_completed', status: 409, description: 'A concurrent request already completed this token rotation within the five-second grace window. The winning replacement remains valid; this response is not retryable.' },
    ...
    { type: 'idempotency_key_required', status: 400, description: 'Canonical command requires an Idempotency-Key header.' },
    { type: 'project_revision_required', status: 428, description: 'Canonical Project command requires If-Match.' },
    { type: 'project_revision_mismatch', status: 412, description: 'If-Match does not equal the Project current revision.' },
    { type: 'trust_revision_required', status: 428, description: 'Revisioned relationship or recurring command requires If-Match.' },
    { type: 'trust_revision_mismatch', status: 412, description: 'If-Match does not equal the relationship resource current revision.' },
    { type: 'worker_revision_required', status: 428, description: 'Revisioned Worker profile command requires If-Match.' },
    { type: 'worker_revision_mismatch', status: 412, description: 'If-Match does not equal the Worker resource current revision.' },
    { type: 'worker_catalogue_revision_mismatch', status: 412, description: 'If-Match does not equal the selected catalogue serviceVersion.' },
    { type: 'capability_unavailable', status: 503, description: 'The named server-authoritative capability is disabled in this release.' },
  ],
};

applyGroundedOpenApi(spec);
