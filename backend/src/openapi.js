...
const spec = {
  ...
  info: {
    ...
    description: [
      ...
      '- Customer data safety: public labourer search returns approximate coordinates only. Labourers do not receive exact customer address, exact job coordinates, notes, or customer phone until a booking is `accepted` or `in_progress`. Customers can always see the address they entered.',
    ].join('\n'),
    ...
    schemas: {
      ...
      LabourerCandidate: {
        ...
        properties: {
          ...
          approx_lat: { type: 'number', nullable: true, description: 'Rounded approximate current latitude, not enough for doorstep-level tracking.' },
          approx_lng: { type: 'number', nullable: true, description: 'Rounded approximate current longitude, not enough for doorstep-level tracking.' },
          location_precision: { type: 'string', enum: ['approximate'], nullable: true },
          distance_km: { type: 'number' },
        },
      },
      MatchRequest: {
        type: 'object',
        description: 'Customers see their own entered address/location. Pinged labourers see approximate coordinates only until a booking is accepted and should use the resulting Booking resource for exact reveal state.',
        properties: {
          id: { type: 'string', format: 'uuid' },
      ...
      Booking: {
        type: 'object',
        description: 'Viewer-dependent booking shape. Customers keep their entered address/location. Labourers receive exact `address`, `location_lat`, `location_lng`, `notes`, and `customer_phone` only when status is `accepted` or `in_progress`; before that, labourer views may include `approx_lat`/`approx_lng` instead and exact fields may be absent.',
        properties: {
          id: { type: 'string', format: 'uuid' },
          customer_id: { type: 'string', format: 'uuid' },
          ...
          notes: { type: 'string', nullable: true },
          customer_phone: { type: 'string', nullable: true, description: 'Only present to labourers once exact contact is revealed (`accepted` or `in_progress`).' },
          labourer_phone: { type: 'string', nullable: true, description: 'Only present to customers once exact contact is revealed (`accepted` or `in_progress`).' },
          labourer_current_lat: { type: 'number', nullable: true, description: 'Fresh exact labourer live latitude, only present to the customer after contact reveal.' },
          labourer_current_lng: { type: 'number', nullable: true, description: 'Fresh exact labourer live longitude, only present to the customer after contact reveal.' },
          created_at: { type: 'string', format: 'date-time' },
        },
      },
      WebhookSubscription: {
              ...
              'application/json': {
                schema: {
                  ...
                  properties: {
                    ...
                    name: { type: 'string' },
                    id_last4: { type: 'string', nullable: true, description: 'Last four digits of the SA ID. Raw ID and derived DOB/sex/citizenship fields are not returned.' },
                    vendor: { type: 'object', nullable: true, properties: { request_id: { type: 'string' } } },
                  },
                },
              },
            },
