...
- **Mac mini stays alive as dev / staging.** Cutover is additive, not destructive. `launchctl kickstart -k gui/$(id -u)/com.togt.backend` continues to work after deploy. Dev API keys remain in the Mac's `togt` DB / password manager and must not be written into deployment docs.
